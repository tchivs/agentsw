...
const messages = {
  "root.description": {
    en: "Manage OpenAI/Anthropic-protocol model providers and sync them into coding agents\n(Claude Code, Codex, omp, pi, prime-agent, opencode, Hermes, WorkBuddy). Model metadata\n(context window, input/output limits, reasoning levels) is enriched from models.dev.\n\nRun without arguments (or via `npx smart-switch`) for the interactive menu.",
    "zh-CN": "管理 OpenAI / Anthropic 协议的模型供应商并同步到编码智能体\n(Claude Code、Codex、omp、pi、prime-agent、opencode、Hermes、WorkBuddy)。模型元数据\n(上下文、输入/输出上限、推理等级)由 models.dev 补全。\n\n不带参数运行(或 `npx smart-switch`)即可打开交互菜单。",
  },
  "help.language": { en: "UI language: en | zh-CN (also SMART_SWITCH_LANG)", "zh-CN": "界面语言: en | zh-CN (也可用 SMART_SWITCH_LANG)" },
  ...
  "cmd.prune": { en: "remove a provider's entries from app configs (keeps it in smart-switch)", "zh-CN": "从应用配置移除供应商条目(仍保留在 smart-switch)" },
  ...
  "add.models": { en: "model ids (comma separated)", "zh-CN": "模型 id(逗号分隔)" },
  ...
  "import.noneFound": { en: "no custom providers found in supported app configs", "zh-CN": "支持的应用配置中未发现自定义供应商" },
  ...
  "import.discoveryFailed": { en: "{id}: no models in configs and discovery failed ({error}) — use smart-switch add", "zh-CN": "{id}:配置中无模型且发现失败({error})——请改用 smart-switch add" },
  ...
  "import.next": { en: "next: smart-switch use {id}", "zh-CN": "下一步: smart-switch use {id}" },
  ...
  "add.next": { en: "next: smart-switch use {id}", "zh-CN": "下一步: smart-switch use {id}" },
  ...
  "list.none": { en: "no providers configured (config: {file})\nrun: smart-switch add or smart-switch import", "zh-CN": "尚未配置供应商(配置:{file})\n请运行 smart-switch add 或 smart-switch import" },
  ...
  "sync.noActive": { en: "no active provider; run smart-switch use <id> first", "zh-CN": "没有当前供应商;请先运行 smart-switch use <id>" },
