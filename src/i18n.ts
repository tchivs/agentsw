...
const messages = {
  ...
  "menu.rename": { en: "rename provider         (migrate references with backups)", "zh-CN": "重命名供应商            （备份并迁移引用）" },
  "menu.renameProvider": { en: "which provider to rename?", "zh-CN": "重命名哪个供应商？" },
  "menu.newId": { en: "new provider ID", "zh-CN": "新的供应商 ID" },
  "menu.renameConfirm": { en: "rename {oldId} to {newId} across configured apps?", "zh-CN": "将 {oldId} 重命名为 {newId}，并迁移已配置智能体的引用？" },
  "menu.removeScope": { en: "where should this provider be removed?", "zh-CN": "从哪里删除供应商？" },
  "menu.removeStore": { en: "agentsw store only (keep app configs)", "zh-CN": "仅从 agentsw 删除（保留智能体配置）" },
  "menu.removeEverywhere": { en: "agentsw and matching app configurations", "zh-CN": "从 agentsw 和匹配的智能体配置中删除" },
  "menu.removeLocal": { en: "one agent only (including entries not imported into agentsw)", "zh-CN": "仅从一个智能体删除（包括未导入 agentsw 的供应商）" },
  "menu.noRemovable": { en: "no providers found in this scope", "zh-CN": "这个范围内没有可删除的供应商" },
  "menu.removeApp": { en: "which agent should this provider be removed from?", "zh-CN": "从哪个智能体删除供应商？" },
  "menu.removeConfirmScope": { en: "remove {id} from {scope}? Default references will be cleared; files are backed up first.", "zh-CN": "从 {scope} 删除 {id}？相关默认引用将被清理，文件会先备份。" },
